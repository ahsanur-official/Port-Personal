...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const initialActive =
    tabLinks.find((btn) => btn.classList.contains("active"))?.dataset.tab ||
...
function showNotification(title, message, type = "info", duration = 5000) {
  ...
  closeBtn.addEventListener("click", () => {
    notification.style.animation =
      "slideInRight 0.3s cubic-bezier(0.4, 0, 0.2, 1) reverse";
    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting) {
          Object.values(navAnchorMap).forEach((link) =>
...
document.querySelectorAll(".achievement-box .box-image").forEach((img) => {
  img.addEventListener("click", () =>
    showImage(img.dataset.full || img.src, img.alt),
  );
});
...
const featuredSpotlightImageEl = document.getElementById(
  "featuredSpotlightImage",
);
const featuredSpotlightMetaEl = document.getElementById(
  "featuredSpotlightMeta",
);
const featuredSpotlightKickerEl = document.getElementById(
  "featuredSpotlightKicker",
);
const featuredSpotlightTitleEl = document.getElementById(
  "featuredSpotlightTitle",
);
const featuredSpotlightDetailsEl = document.getElementById(
  "featuredSpotlightDetails",
);
const featuredSpotlightTagsEl = document.getElementById(
  "featuredSpotlightTags",
);
const featuredSpotlightLiveEl = document.getElementById(
  "featuredSpotlightLive",
);
const featuredSpotlightDetailsBtnEl = document.getElementById(
  "featuredSpotlightDetailsBtn",
);
...
const i18n = {
  en: {
    ...
    heroBody:
      "Based in Bangladesh and finishing my CSE degree at Pundra University of Science and Technology, I blend machine learning, analytics storytelling, and front-end craft to ship reliable, insight-led products people can use and trust.",
    heroSub:
      "I help founders and student teams turn messy data into dashboards, ML prototypes, and shippable web experiences.",
    ...
    aboutP1:
      "I'm an aspiring data scientist who turns messy data into models and dashboards. I started with algorithms and C/C++, leveled up with Python analytics, and now build ML prototypes alongside polished web experiences.",
    aboutP2:
      "My focus is supervised learning, feature engineering, and experiment design with a bias for explainability and clean handoff. I ship data stories, lightweight APIs, and UIs that make insights actionable.",
    aboutP3:
      "Comfortable with Pandas, NumPy, scikit-learn, SQL, and visualization; currently exploring MLOps basics and deep learning for next-step projects.",
  ...
  bn: {
    ...
    heroBody:
      "বাংলাদেশে অবস্থান করে এবং পুন্দ্র ইউনিভার্সিটিতে CSE শেষ করার পথে আমি machine learning, analytics storytelling এবং front-end craft একসাথে ব্যবহার করে ব্যবহারযোগ্য ও নির্ভরযোগ্য প্রোডাক্ট তৈরি করি।",
    heroSub:
      "আমি প্রতিষ্ঠাতা ও স্টুডেন্ট টিমকে জটিল ডাটা থেকে ড্যাশবোর্ড, এমএল প্রোটোটাইপ ও ব্যবহারযোগ্য ওয়েব এক্সপেরিয়েন্স তৈরি করতে সাহায্য করি।",
    ...
    letsWorkSub:
      "ডাটা সায়েন্স প্রজেক্ট, এমএল প্রোটোটাইপ, রিসার্চ সহযোগিতা এবং প্রোডাক্ট ডিজাইন কাজের জন্য আমি উন্মুক্ত। সাধারণত ২৪ ঘণ্টার মধ্যে উত্তর দিই।",
    ...
    aboutP1:
      "আমি একজন aspiring data scientist, যে জটিল ডাটাকে মডেল ও ড্যাশবোর্ডে রূপ দিই। অ্যালগরিদম ও C/C++ থেকে শুরু করে Python analytics-এ এগিয়ে এখন ML prototype ও polished web experience তৈরি করি।",
    aboutP2:
      "আমার মূল ফোকাস supervised learning, feature engineering এবং experiment design; explainability ও clean handoff আমার অগ্রাধিকার।",
    aboutP3:
      "Pandas, NumPy, scikit-learn, SQL ও visualization-এ কাজ করি; পাশাপাশি MLOps basics ও deep learning শিখছি।",
...
const renderProjectsFromData = () => {
  ...
  projectGridEl.innerHTML = appState.projects
...
const renderFeaturedProjects = () => {
  ...
  if (!featured.length) {
    ...
    document.dispatchEvent(new CustomEvent("projects:featured-updated"));
    return;
  }

  featuredTrackEl.innerHTML = featured
    .map(
      (project) => `
  ...
  appState.featuredIndex = Math.min(
    appState.featuredIndex,
    featured.length - 1,
  );
  renderFeaturedSpotlight(
    featured[appState.featuredIndex],
    appState.featuredIndex,
    featured.length,
  );
...
const renderFeaturedSpotlight = (project, index = 0, total = 0) => {
  ...
  if (featuredSpotlightTitleEl)
    featuredSpotlightTitleEl.textContent = getField(project, "title");
  if (featuredSpotlightDetailsEl)
    featuredSpotlightDetailsEl.textContent = getField(project, "details");
  if (featuredSpotlightTagsEl) {
    featuredSpotlightTagsEl.innerHTML = (project.tags || [])
      .map((tag) => `<span>${tag}</span>`)
      .join("");
  }
...
const setLanguageUI = () => {
  ...
  navMap.forEach(([href, key]) => {
    const el = document.querySelector(
      `.nav-links a[href='${href}'] .nav-label`,
    );
  ...
  const infoLabels = document.querySelectorAll(
  ...
  const blogNav = document.querySelector(
    ".nav-links a[href='Blogs/index.html'] .nav-label",
  );
  if (blogNav) blogNav.textContent = t("blogLabel");

  const blogFooter = document.querySelector(
    ".footer-links a[href='Blogs/index.html']",
  );
  ...
  const formTimelineLabel = document.querySelector(
    "label[for='contact-timeline']",
  );
  const formMessageLabel = document.querySelector(
  ...
  const sendBtnSpan = document.querySelector(
  ...
  const featuredSpotlightKicker = document.getElementById(
    "featuredSpotlightKicker",
  );
  if (featuredSpotlightKicker)
    featuredSpotlightKicker.textContent = t("featuredSpotlightKicker");
  const searchInput = document.getElementById("projectSearch");
  if (searchInput) searchInput.placeholder = t("searchPlaceholder");
  if (projectsPrevEl)
    projectsPrevEl.setAttribute("aria-label", t("projectCarouselPrev"));
  if (projectsNextEl)
    projectsNextEl.setAttribute("aria-label", t("projectCarouselNext"));
  ...
  if (languageToggleEl) {
    languageToggleEl.textContent =
      appState.language === "bn" ? "BN/EN" : "EN/BN";
    languageToggleEl.setAttribute(
      "aria-label",
      appState.language === "bn" ? "Switch to English" : "Switch to Bangla",
    );
  }
};
...
const openProjectDrawer = (projectId) => {
  ...
  if (problem)
    problem.textContent = `${t("drawerProblem")} ${getField(project, "problem")}`;
  if (solution)
    solution.textContent = `${t("drawerSolution")} ${getField(project, "solution")}`;
  if (details)
    details.textContent = `${t("drawerDetails")} ${getField(project, "details")}`;
  if (tags) {
...
document.addEventListener("click", (e) => {
  ...
  const titleText =
    card.querySelector(".project-copy h3")?.textContent?.trim() || "Project";
  const blurbs = card.querySelectorAll(".project-blurb");
  const problemText =
    blurbs[0]?.textContent?.replace(/^Problem:\s*/i, "").trim() || "";
  const solutionText =
    blurbs[1]?.textContent?.replace(/^Solution:\s*/i, "").trim() || "";
  const detailsText = detailsBtn.getAttribute("data-details") || "";
  const tagsText = Array.from(card.querySelectorAll(".project-tags span")).map(
  ...
  if (live)
    live.href = card.querySelector(".live-link")?.getAttribute("href") || "#";
  if (repo) {
...
if (projectDrawerEl) {
  projectDrawerEl.addEventListener("click", (e) => {
    if (
      e.target.classList.contains("project-drawer") ||
      e.target.classList.contains("project-drawer-backdrop")
    ) {
      closeProjectDrawer();
    }
  });
...
if (projectsPrevEl && projectGridEl) {
  projectsPrevEl.addEventListener("click", () => {
    projectGridEl.scrollBy({
      left: -Math.max(projectGridEl.clientWidth * 0.82, 320),
      behavior: "smooth",
    });
  });
}
...
const renderFeaturedOrderList = () => {
  ...
  const featured = getFeaturedProjects();
  featuredOrderListEl.innerHTML = featured
    .map(
      (project) => `
  ...
  let draggingId = "";
  featuredOrderListEl
    .querySelectorAll(".featured-order-item")
    .forEach((item) => {
      ...
      item.addEventListener("drop", (e) => {
        ...
        const sourceEl = featuredOrderListEl.querySelector(
          `[data-project-id='${draggingId}']`,
        );
        const targetEl = featuredOrderListEl.querySelector(
...
const applyFeaturedOrderFromPanel = () => {
  ...
  const orderMap = {};
  Array.from(
    featuredOrderListEl.querySelectorAll(".featured-order-item"),
  ).forEach((item, index) => {
  ...
  showNotification(
    ...
    "Featured project order applied successfully.",
    "success",
    3200,
  );
};
...
const resetFeaturedOrder = () => {
  ...
  appState.projects = appState.projects.map((project) => {
    ...
    return {
      ...project,
      order:
        appState.originalFeaturedOrderMap[project.id] || project.order || 9999,
    };
  });
  appState.featuredIndex = 0;
  renderFeaturedProjects();
  ...
  try {
    ...
    if (Array.isArray(data) && data.length) {
      ...
      appState.projects.forEach((project, idx) => {
        if (project.featured) {
          appState.originalFeaturedOrderMap[project.id] =
            Number(project.order) || idx + 1;
        }
      });
      ...
      if (errors.length) {
        ...
        showNotification(
          ...
          `${errors.length} schema issue(s): ${preview}`,
          "error",
          7000,
        );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyProjectFilters = () => {
    ...
    projectCards.forEach((card) => {
      ...
      const categoryMatch =
        activeCategory === "all" || getCardCategory(card) === activeCategory;
  ...
  const setupProjectAnimations = () => {
    const cards = Array.from(
      document.querySelectorAll(
        ".projects-grid .project-card, .featured-track .featured-card",
      ),
    );
    ...
    cards.forEach((card, index) => {
      ...
      const revealDistance = isFeaturedCard ? 22 : 46;
      const revealRotate = isFeaturedCard ? 1.2 : 2.8;

      card.style.setProperty("--reveal-x", `${direction * revealDistance}px`);
      card.style.setProperty("--reveal-rotate", `${direction * revealRotate}deg`);
      card.style.setProperty("--reveal-scale", isFeaturedCard ? "0.985" : "0.97");
      card.style.animationDelay = `${index * 110}ms`;

      if (!isFeaturedCard && canHover) {
        const resetParallax = () => {
          card.style.setProperty("--card-drift-x", "0px");
          card.style.setProperty("--card-drift-y", "0px");
        ...
        const updateParallax = (event) => {
          ...
          const driftX = Math.max(-12, Math.min(12, deltaX * 22));
          const driftY = Math.max(-8, Math.min(8, deltaY * 18));
          const tilt = Math.max(-2.2, Math.min(2.2, deltaX * 5));
        ...
        card.addEventListener("pointerenter", updateParallax);
        card.addEventListener("pointermove", updateParallax);
        card.addEventListener("pointerleave", resetParallax);
        card.addEventListener("blur", resetParallax, true);
      }
  ...
  const buildProjectFilterButtons = () => {
    ...
    items.forEach((category) => {
      ...
      btn.textContent =
        category === "all" ? getText("all") : getCategoryLabel(category);
  ...
  const openQuickPanel = () => {
    ...
    if (quickFab) {
      quickFab.classList.add("open");
      quickFab.setAttribute("aria-expanded", "true");
    }
  };
  ...
  document.addEventListener("projects:featured-updated", () => {
    setupProjectAnimations();
  });
  ...
  if (quickFab) {
    quickFab.setAttribute("aria-expanded", "false");
    quickFab.setAttribute("aria-controls", "quickActionsPanel");
